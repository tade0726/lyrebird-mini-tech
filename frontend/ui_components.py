...
class UIComponents:
    ...
    def render_sidebar_preferences(self):
        ...
        with st.spinner("Loading your preferences..."):
            ...
            if preferences:
                ...
                st.info("No preferences found yet. Submit edits to generate preferences.")
    ...
    def render_audio_recorder(self):
        ...
        st.write("Click to start recording audio, then click again to stop and transcribe.")
    ...
    def _process_recorded_audio(self, audio_bytes: bytes):
        ...
        with st.spinner("Transcribing audio..."):
            result = self.api_client.send_audio_to_dictation(audio_bytes, file_extension=".wav")
    ...
    def _process_uploaded_audio(self, uploaded_file):
        ...
        with st.spinner("Transcribing uploaded audio..."):
            result = self.api_client.send_audio_to_dictation(audio_bytes, file_extension=file_extension)
            ...
            if result["success"]:
                ...
                st.error(result.get("error", "An unknown error occurred during transcription."))
    ...
    def render_transcript_section(self):
        ...
        has_transcript = bool(st.session_state.transcript or st.session_state.edited_transcript)
    ...
    def _render_original_transcript_only(self):
        ...
        st.info("Original transcript available but no formatted version was provided by the API.")
        st.text_area(
            "Original Transcript",
    ...
    def _render_action_buttons(self, edited_text: str):
        ...
        with col2:
            if st.button("📤 Submit", disabled=not st.session_state.transcript, use_container_width=True):
                self._handle_submit_edit(edited_text)
        
        with col3:
            if st.button("🗑️ Clear", disabled=not st.session_state.formatted_transcript, use_container_width=True):
    ...
    def _handle_submit_edit(self, edited_text: str):
        ...
        with st.spinner("Analyzing preferences..."):
            result = self.api_client.submit_edit(st.session_state.transcript, edited_text)
